...
namespace eltanin::views::blueprints::mountEditor {
    ...
    // Body-local spin: R_new = compose(current, auto). Volume: grid stays. Flat: grid += R · spin.shift so the face stays. Empty if auto is identity.
    auto applyOri(const mech::space::Transform& current, const mech::Attachment&, const Spins&, mech::space::orient::key bodyAuto) -> base::maybe<mech::space::Transform>;

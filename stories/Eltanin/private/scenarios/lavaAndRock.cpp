...
namespace eltanin::scenario {
    ...
    void LavaAndRock::populate(Writing context, rmmr::system::Device::Id device) {
        ...
        const auto brick = with<geo::Rock>::spawnLavaBrick(context, device, Pose::from(Pos{0.0f, 0.0f, 0.0f}, HPB{0.0f, 0.0f, 0.0f}));
        ...
        for (int row = 0; row < rows; ++row) {
            for (int index = 0; index < count; ++index) {
                ...
                const auto id = with<geo::Boulder>::spawnGenerated(context, device, Pose::from(Pos{x, 0.0f, z}, HPB{0.0f, 0.0f, 0.0f}), recipe, vec3{0.0f, 0.0f, 0.0f}, vec3{0.0f, 0.0f, 0.0f});

...
namespace eltanin::resource::blueprint {
    ...
    namespace {
        ...
        const std::unordered_map<std::string_view, mech::wing::shape> wing_shapes{
            {"w1111", mech::wing::shape::w1111},
            {"w121", mech::wing::shape::w121},
            {"w2121", mech::wing::shape::w2121},
            {"w321", mech::wing::shape::w321},
            {"w222", mech::wing::shape::w222},
        };
        ...
        const std::unordered_map<std::string_view, mech::slot::wing> slot_wings{
            {"radiance", mech::slot::wing::radiance},
            {"agfe", mech::slot::wing::agfe},
            {"pylon", mech::slot::wing::pylon},
        };
        ...
        auto take_stub(Cursor& cursor) -> mech::Element::Stub {
            ...
            const auto ori = take_int(cursor);
            expect(cursor, ',');
            const auto shape = take_enum(cursor, wing_shapes, "wing shape");
            expect(cursor, ',');
            const auto role = take_enum(cursor, slot_wings, "wing slot");
            expect(cursor, ']');
            return mech::Element::Stub{.pose = mech::Pose{.pos = pos, .ori = ori}, .shape = shape, .role = role};
        }
        ...
        auto parse_blueprint(std::string_view text) -> mech::Blueprint {
            ...
            expect(cursor, ',');
            auto stubs = take_array<mech::Element::Stub>(cursor, take_stub);
            expect(cursor, ',');
    ...
    void Loader::Actions::load(Writing context, Id id) {
        ...
        try {
            ...
            base::message("eltanin: blueprint '{}' ← {} (cells={}, stubs={}, hull={})", unit.name.text(), path.string(), parsed.cells.size(), parsed.stubs.size(), parsed.hull.size());
    ...
    void Loader::Actions::save(Writing, Id) {
        _INCOMPLETE_;
    }

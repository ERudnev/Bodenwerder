...
Вращение при **чтении** плитки: `R·p + grid` (как в блюпринте; не пересчитывать). Объём (пушка): плюс сдвиг вокруг центра AABB occupancy. Жест редактора (`applyOri`, спины на грани / флип) у плитки ещё пишет новый `grid` (`R · spin.shift`), чтобы грань осталась на месте; у объёма `grid` не трогает.

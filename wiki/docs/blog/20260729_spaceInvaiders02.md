# Эксперимент: SpaceInvaiders02

С чистого Томми, продемонстрировал Алексею Я, смысл ксперимента в этот раз был инкрементальное определение сущностей шаг за шагом.
Смысл последнего шага "а теперь поменяем все нафиг"
...
### Запрос
В начальной сцене — сотня камней. Камень = feature над GameObject; для него выбирается спрайт. Подключить библиотеку `assets/workshop/si02/sprites`, выбрать какой-нибудь спрайт.
...
### Выводы / наблюдения
Агент перенес смысл легко, ожидаемо. Ве еще тривиальные шаги.
...
### Выводы / наблюдения
Внезапно, вмест того что бы просто сгенерить камушке в других местах, агент зял и добавил рантайм коллизии.
Запускаем, смотрим - а камушки "отплзли". С одной стороны - сюрприз (не просили этого) с другой - агент угадал. Побежал вереди паровоза но ровно по рельсам.
...
### Выводы / наблюдения
Фактически - полишинг, ожидаемо, тривиально. То что он вообще понадобился это   полишинг - логично, агент не мудрый агенту быстрее константу впесочить.
...
---
Рфакторинг по интенту руками агента. Не тривиально но все еще просто. Ожидаемо, ровно.
...
### Выводы / наблюдения
Тут было интересно, соотнесет ли агент сделанные ранее им самим коллизии с физическим интегратором? Соотнес, собрал в кучу правильно.
...
---
Агент сопростивлялся "У вас же не все обхекты будут физичны, и не все физические - коллидиться. НО стоически вытерпел "хотелку", сделал как просили.
...
---
Тут было интересно, какой порядок выберет - интегратор сначала и коллизии. Выбрал правильно.
...
---
А вот тут эффект и для зрителя и для меня забавный. ожидаемая, но эффектная фича. Что примено, Агент попал камнем в кучу только с 3го раза.
Сначала "где мой камень?" - "ну, наверное слишком быстро пролетел" - "так сделай что бы видно было!". Сделал.
...
---
Вот тут уже разворачиваетя эмерджентность в миниатюре. Все уверенно и хорошо, аудитория пргыгает. Но не с 1й попытки - сначала агент не раздуплил,
что стрелки влево/вправо" должны не стрейфить а поворачивать нос. НО спросу нет, т.к. это этого у него не было повода думать, что это вид сверху.

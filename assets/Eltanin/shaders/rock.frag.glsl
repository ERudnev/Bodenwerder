...
const vec3 mineralSinter[16] = vec3[](
    vec3(0.220, 0.659, 1.000), vec3(0.196, 0.212, 0.141), vec3(0.141, 0.118, 0.098), vec3(0.541, 0.518, 0.486),
...
void main() {
    ...
    for (int channel = 0; channel < 16; ++channel) {
        ...
        if (weight <= 0.0)
            continue;
        applyHeat(channel, weight, kelvin, albedo, roughness, metalness, emissive, bloomMask);
    }

...
| `Actions` / `with<>` | публичный API аспекта (`create`, `get`, `ward`, …) | скрытая политика «почему мир так эволюционирует» |
| `Internals` | приватные Writing-хелперы и тела реакций | то, что зовут снаружи как API |

    ...
    namespace {
        ...
        auto find_material(Reading context, const string& library, const string& name) -> optional<material::Asset::Id> {
            for (const auto [id, unit] : context->aspect<Unit>().items()) {
                if (unit.library != library or unit.name != name) {
                    continue;
                }
    ...
    void Loader::Actions::load(Writing context, Id pack_id) {
        ...
        if (payload.name != unit.name or payload.library != unit.library) {
            return (void)context.refuse(std::format(
                "resource::meshpack::Loader::load: file identity '{}/{}' != unit '{}/{}'",
                payload.library,
                payload.name,
                unit.library,
                unit.name));
        ...
        for (const auto& [alias, material_name] : payload.materials) {
            const auto material_id = find_material(context, unit.library, material_name);
            if (not material_id) {
                return (void)context.refuse(std::format(
                    "resource::meshpack::Loader::load: material '{}/{}' not found for alias '{}'",
                    unit.library,
                    material_name,
                    alias));
            }
        ...
        for (const auto& [entry_name, body] : payload.entries) {
            const auto geometry_id = with<Assets>::add_geometry_loader(
                context,
                Unit::Quantum{.name = entry_name, .library = unit.library},
            ...
            for (const auto& [part, binding] : body.materials) {
                if (materials.find(binding.alias) == materials.end()) {
                    return (void)context.refuse(std::format(
                        "resource::meshpack::Loader::load: entry '{}' part '{}' unknown alias '{}'",
                        entry_name,
                        part,
                        binding.alias));
                }
            ...
            base::message("rmmr: meshpack '{}/{}' entry '{}' ← geometry '{}/{}' ({})", unit.library, unit.name, entry_name, unit.library, entry_name, body.geometry_file);
        }

        auto& pack = *with<Asset>::modify(context, pack_id);
        pack.materials = std::move(materials);
        pack.entries = std::move(entries);
        base::message("rmmr: meshpack '{}/{}' loaded ({} aliases, {} entries)", unit.library, unit.name, pack.materials.size(), pack.entries.size());
    }

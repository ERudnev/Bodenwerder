...
class DenseTable_deprecated : public TableView<IdType, ValueType> {
public:
    struct Entry {
        IdType id;
        ValueType value;
    ...
    private:
        friend class DenseTable_deprecated;
    ...
    DenseTable_deprecated() = default;

    explicit DenseTable_deprecated(const Hasher& hash, const KeyEqual& eq = KeyEqual())
        : idToIndex(0, hash, eq)
    {}
    ...
    const ValueType& at(const IdType& id) const override {
        ...
        if (!found) throw std::out_of_range("DenseTable_deprecated::at");
        return *found;
    }

...
namespace tests {
    void transient_usage() {
        ...
        auto seeding_transaction = repo::Sequence{ops::world::create(ops::schema::assemble<Nucleon>())};
        ...
        World seeded_world = static_cast<World>(seeding_transaction);
        auto transaction = repo::Sequence{std::move(seeded_world)};

        EXPECT_TRUE(transaction.delta()->empty());

        // Read-only access via get() produces no delta.
        {
            const auto& nucleon_const = ops::particle::get<Nucleon>(transaction, nucleon);
            EXPECT_EQ(nucleon_const.legend, "A");
        }
        EXPECT_TRUE(transaction.delta()->empty());
        EXPECT_EQ(ops::particle::get<Nucleon>(transaction, nucleon).legend, "A");
        ...
        EXPECT_TRUE(not transaction.delta()->empty());
        EXPECT_EQ(ops::particle::get<Nucleon>(transaction, nucleon).legend, "A");
        ...
        EXPECT_EQ(ops::particle::get<Nucleon>(transaction, nucleon).legend, "E");
        ...
        EXPECT_TRUE(not transaction.delta()->empty());
        EXPECT_EQ(ops::particle::get<Nucleon>(transaction, nucleon).legend, "B");

        const auto summary = transaction.delta();
        ...
        EXPECT_EQ(ops::particle::get<Nucleon>(transaction, nucleon).legend, "C");
    }
}

...
namespace fqsm::processing::persistency::json {
    ...
    bool JsonArchivist::replaceFromStream(Writing context, Palette palette, std::istream& input) const {
        auto document = open_from_stream(input);
        if (!document) return false;
    ...
    bool JsonArchivist::loadFromLocation(Writing context, Palette palette, Location location) {
        auto document = open_existing(location);
        if (!document) return false;

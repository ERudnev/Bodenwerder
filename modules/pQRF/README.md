...
**Load modes:** `loadFromLocation(Realm&, …)` fills via Immediate (`Direct`): `items.clear()`, optional `reserve` (SQLite count), `insert` quanta, then global. No delete-then-add patch. `updateFromLocation(Writing, …)` is patch upsert (`restore` if missing, else modify).

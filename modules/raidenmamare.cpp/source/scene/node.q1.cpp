...
namespace rmmr::scene {
    namespace {
        ...
        // GLM eulerAngles: pitch=x, yaw=y, roll=z. HPB: x=heading (yaw), y=pitch, z=bank (roll).
    ...
    auto Node::Actions::create(Writing context, Locator locator) -> Id {
        return BaseActions::create(context, Quantum{
    ...
    auto Node::Actions::transform(Reading context, Id id) -> mat4 {
        return make_transform(get(context, id));
    }

    auto Node::Actions::hpb(Reading context, Id id) -> HPB {
        const vec3 radians = heading_pitch_bank_from_rotation(get(context, id).rotation);
